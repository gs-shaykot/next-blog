...
export default function AIContentCreator() {
    ...
    const handleGenerateArticle = async () => {
        ...
        try {
            ...
            if (res.ok) {
                setArticleResult(data.article);
                Swal.fire('Success!', 'Article generated successfully.', 'success');
            } else {
                Swal.fire('Failed', data.message || 'Failed to generate article.', 'error');
            }
        } catch (err) {
            console.error(err);
            Swal.fire('Error', 'An error occurred while generating the article.', 'error');
        } finally {
    ...
    const handleGenerateImage = async () => {
        ...
        try {
            ...
            if (res.ok && data.base64Image && data.mimeType) {
                ...
                Swal.fire('Success!', 'Image generated successfully.', 'success');
            } else {
                Swal.fire('Failed', data.message || 'Image generation failed.', 'error');
            }
        } catch (err) {
            console.error(err);
            Swal.fire('Error', 'An error occurred while generating the image.', 'error');
        } finally {
    ...
    const handleUploadToCloudinary = async () => {
        ...
        try {
            ...
            Swal.fire('Uploaded!', 'Image uploaded successfully to Cloudinary.', 'success');
        } catch (err) {
    ...
    const handleTransfer = () => {
        ...
        Swal.fire('Success!', 'Data transferred to editor.', 'success').then(() => {
            ...
            <div className='w-full mx-auto space-y-12 p-6'>
                ...
                <div className='border rounded-lg p-6 shadow-sm w-11/12 mx-auto'>
                    ...
                    <button
                        ...
                        className='w-full bg-blue-500 text-white py-2 rounded hover:bg-blue-600 disabled:bg-blue-300 transition'
                ...
                <div className='border rounded-lg p-6 shadow-sm w-11/12 mx-auto'>
                    ...
                    <button
                        ...
                        className='w-full bg-green-500 text-white py-2 rounded hover:bg-green-600 disabled:bg-green-300 transition'
                    ...
                    {imageResult && !loadingImage && (
                        <div className='mt-4'>
                            <img
                                ...
                                onError={() => Swal.fire('Error', 'Failed to load image.', 'error')}
                            />
                            <div className='flex justify-between items-center gap-3'>
                                <button
                                    onClick={handleDownloadImage}
                                    className='mt-3 w-full md:w-6/12 bg-gray-700 text-white py-2 rounded hover:bg-gray-800 transition'
                                >
                                    Download Image
                                </button>
                                <button
                                    ...
                                    disabled={uploading}
                                    className='mt-3 w-full md:w-6/12 border-2 border-blue-600 text-blue-600 hover:bg-blue-600 hover:text-white rounded py-2 transition'
                                >
                    ...
                    {(articleResult || imageResult) && (
                        <button
                            ...
                            type='button'
                            className='w-full font-medium transition duration-200 cursor-pointer rounded border-2 border-blue-600 bg-blue-600 text-white py-2 mt-4 hover:bg-blue-700'
                        >
